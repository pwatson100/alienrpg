...
export default class AlienRPGCTContext extends foundry.applications.sidebar.tabs.CombatTracker {
  ...
  // /** @override */
	...
	// 	const { config } = CONFIG.ALIENRPG.CombatTracker;
	// 	if (typeof config === 'object') return config;
	// 	else {
	// 		try {
	// 			const { src } = CONFIG.ALIENRPG.CombatTracker;
	...
	// 				cfg.buttons.unshift(...ALIENRPG.CombatTracker.DefaultCombatantControls.slowAndFastActions);
	// 			} else if (game.settings.get(MODULE_ID, SETTINGS_KEYS.SINGLE_ACTION)) {
	// 				cfg.buttons.unshift(...ALIENRPG.CombatTracker.DefaultCombatantControls.singleAction);
	// 			}

	// 			if (game.settings.get(MODULE_ID, SETTINGS_KEYS.RESET_EACH_ROUND)) {
	// 				cfg.buttons.unshift(...ALIENRPG.CombatTracker.DefaultCombatantControls.lockInitiative);
	// 			}

	// 			CONFIG.ALIENRPG.CombatTracker.config = cfg;
	// 			return cfg;
	// 		} catch (error) {
	// 			console.error(error);

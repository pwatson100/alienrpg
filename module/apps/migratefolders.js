...
export default async function migrateFolders() {
	...
	const acFolderIDs = [
		{
			folderName: "Alien Creature Tables",
			folderID: "LrpFZIuICZfNAr2v",
			folderType: "RollTable",
		},
		{
			folderName: "Alien Mother Tables",
			folderID: "A0N3Ct1TQq9BuGk6",
			folderType: "RollTable",
		},
		{
			folderName: "Alien Sub-Tables",
			folderID: "w3xO69hCmTtYDF8d",
			folderType: "RollTable",
		},
		{
			folderName: "Alien Tables",
			folderID: "Btepu5tifRV0Pj7w",
			folderType: "RollTable",
		},
	]
...
async function ReImportFolders(adventurePack, adventurePackName) {
	...
	if (toCreate) {
		for (const [documentName, createData] of Object.entries(toCreate)) {
			...
			const c = await cls.createDocuments(createData, {
				keepId: true,
				keepEmbeddedId: true,
				renderSheet: false,
			})
...
async function deleteFolderIfExist(folderName) {
	const delFolder = game.folders.getName(folderName)
	if (delFolder) {
...
async function deleteJournalIfExist(journalName) {
	const delJournal = game.journal.getName(journalName)
	if (delJournal) {
...
async function deleteIDJournalIfExist(journalID) {
	const delJournal = game.journal.get(journalID)
	if (delJournal) {
		await delJournal.delete()
	}
}
async function deleteTableIfExist(tableName) {
	const delTable = game.tables.getName(tableName)
	if (delTable) {

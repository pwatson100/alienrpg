...
export default class alienrpgSyntheticSheet extends api.HandlebarsApplicationMixin(sheets.ActorSheetV2) {
	...
	_configureRenderOptions(options) {
		...
		switch (game.settings.get("alienrpg", "evolved")) {
			case true:
				...
				// 	)
				// } else {
				options.parts.push("syntheticENheader", "tabs", "syntheticENgeneral", "skills", "inventory", "notes")
				// }
				break
			case false:
				{
					if (game.settings.get("alienrpg", "aliencrt")) {
						...
						options.parts.push("syntheticheader", "tabs", "syntheticgeneral", "skills", "inventory", "notes")
					}
				}
				break
		}
	...
	async _preparePartContext(partId, context) {
		switch (partId) {
			case "syntheticgeneral":
			case "syntheticENgeneral":
			case "skills":
			case "crtuisyntheticgeneral":
	...
	_getTabs(parts) {
		...
		switch (enhanced) {
			case false:
				{
					if (!sheetType) {
						if (!this.tabGroups[tabGroup]) this.tabGroups[tabGroup] = "syntheticgeneral"
					} else {
						if (!this.tabGroups[tabGroup]) this.tabGroups[tabGroup] = "crtuisyntheticgeneral"
					}
				}
				break
			case true:
				if (!this.tabGroups[tabGroup]) this.tabGroups[tabGroup] = "syntheticENgeneral"
				break
		}
		return parts.reduce((tabs, partId) => {
			...
			switch (partId) {
				...
				case "syntheticENheader":
				case "crtuisyntheticheader":
				case "tabs":
					return tabs
				case "notes":
					tab.id = "notes"
					...
					break
				case "syntheticgeneral":
					tab.id = "syntheticgeneral"
					tab.label += "General"
					break
				case "syntheticENgeneral":
					tab.id = "syntheticENgeneral"
					tab.label += "General"
					break
				case "skills":
					tab.id = "skills"
	...
	async _onRender(context, options) {
		...
		logger.debug("Synthetic Sheet derived data:", context)
	}

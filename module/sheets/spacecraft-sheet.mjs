...
export default class alienrpgSpacecraftSheet extends api.HandlebarsApplicationMixin(sheets.ActorSheetV2) {
  ...
  static DEFAULT_OPTIONS = {
    ...
    actions: {
      ...
      ShipPhaseSubmit: this._onShipPhase,
      ...
      ClickDamageLevel: { handler: this._onClickDamageLevel, buttons: [0, 2] },
      RollMajorCD: { handler: this._onRollMajorCD, buttons: [0, 2] },
      RollMinorCD: { handler: this._onRollMinorCD, buttons: [0, 2] },
    },
  ...
  _configureRenderOptions(options) {
    ...
    options.parts.push("tabs", "spacecraftgeneral", "spacecraftinventory", "spacecraftimage", "spacecraftcombat", "notes");
  }
  /* -------------------------------------------- */
  ...
  async _prepareContext(options) {
    ...
    context.sensor_list = CONFIG.ALIENRPG.sensor_list;
    context.pilot_list = CONFIG.ALIENRPG.pilot_list;
    context.gunner_list = CONFIG.ALIENRPG.gunner_list;
    context.engineer_list = CONFIG.ALIENRPG.engineer_list;

    await this._prepareSpaceshipItems(context);
  ...
  async _preparePartContext(partId, context) {
    switch (partId) {
      case "spacecraftgeneral":
      case "spacecraftimage":
      case "spacecraftcombat":
        context.tab = context.tabs[partId];
        break;
      case "spacecraftinventory":
        context.tab = context.tabs[partId];
  ...
  _getTabs(parts) {
    ...
    return parts.reduce((tabs, partId) => {
      ...
      switch (partId) {
        ...
        case "spacecraftinventory":
          tab.id = "spacecraftinventory";
          tab.label += "Modules/Upgrades";
          break;
        case "spacecraftgeneral":
          tab.id = "spacecraftgeneral";
          tab.label += "General";
          break;
        case "spacecraftcombat":
          tab.id = "spacecraftcombat";
          tab.label += "Space Combat Phases";
          break;
        case "spacecraftimage":
          tab.id = "spacecraftimage";
          tab.label += "Image";
          break;
        case "notes":
          tab.id = "notes";
   ...
   */
  async _prepareSpaceshipItems(context) {
    ...
    const inventory = {
      ...
      spacecraftmods: {
        section: "Spacecraft Mods",
        label: game.i18n.localize("ALIENRPG.MODULES-UPGRADES"),
        items: [],
        dataset: { type: "spacecraftmods" },
      },
      spacecraftweapons: {
    ...
    let [items, Weapons, Armor, spacecraftmods, spacecraftweapons] = context.system.items.reduce(
      (arr, item) => {
        // Item details
    ...
    context.critMin = critMin;
    context.critMaj = critMaj;
    await this.actor.update({
      "system.general.critMin": context.critMin.length,
      "system.general.critMaj": context.critMaj.length,
    });
  }
  ...
  async _prepareCrew(sheetData) {
    ...
    sheetData.actor.system.crew.occupants.sort((o1, o2) => {
      const pos1 = CONFIG.ALIENRPG.spacecraft.crewPositionFlags.indexOf(o1.position);
      const pos2 = CONFIG.ALIENRPG.spacecraft.crewPositionFlags.indexOf(o2.position);
  ...
  async _dropCrew(actorId) {
    ...
    if (actorData.type === "vehicles") {
      if (actorData.system.crew.passengerQty >= actorData.system.attributes.passengers.value + 1) {
    ...
    if (actorData.type === "spacecraft") {
      if (actorData.system.crew.passengerQty >= actorData.system.attributes.passengers.value + actorData.system.attributes.crew.value + 1) {
        return ui.notifications.warn(game.i18n.localize("ALIENRPG.fullCrew"));
      }
      return await actorData.addVehicleOccupant(actorId);
    }
  ...
  static async _onShipPhase(event, target) {
    let htmlData = "";
    event.preventDefault();
    const dataset = target.dataset;
    const shipName = this.actor.name;
    const actorID = this.actor.id;
    const element = target.previousElementSibling.selectedOptions[0].innerText;
    const phase = game.i18n.localize(`ALIENRPG.${target.previousElementSibling.name}`);
    ...
    const html = await foundry.applications.handlebars.renderTemplate("systems/alienrpg/templates/chat/ship-combat.hbs", htmlData);
  ...
  static async _onClickDamageLevel(event) {
    event.preventDefault(); // Don't open context menu
    event.stopPropagation(); // Don't trigger other events
    if (event.detail > 1) return; // Ignore repeated clicks
    await this.actor.ClickDamageLevel(this.actor, event.button);
  }

  static async _onRollMajorCD(event, target) {
    event.preventDefault(); // Don't open context menu
    event.stopPropagation(); // Don't trigger other events
    if (event.detail > 1) return; // Ignore repeated clicks
    const dataset = target.dataset;
    ...
    return [
      ...
      },
      {
        ...
        callback: (element) => {
          ...
          // if (itemDel.type === "spacecraft-crit" && this.actor.system.general.critInj.value <= 1) {

...
export default class AlienRPGCTContext extends foundry.applications.sidebar.tabs.CombatTracker {
  _getEntryContextOptions() {
    ...
    return [
      ...
      {
        name: "COMBATANT.ACTIONS.ClearMovementHistory",
        ...
        callback: async (li) => {
          ...
          ui.notifications.info("COMBATANT.CombatantMovementHistoryCleared", { format: { name: combatant.token.name } });
        },
      },
      {
    ...
    async function _swapInitiative(combatant, donerInit, combatants, template) {
      let confirmed = false;
      const myTitle = game.i18n.localize("ALIENRPG.SwapInitiative") + " " + game.i18n.localize("ALIENRPG.ROLLFOR") + " " + `${combatant.name}`;
      const content = await foundry.applications.handlebars
        .renderTemplate(template, {
          combatants: combatants
            .filter(
              (c) => c.initiative && c.id !== combatant.id && (c.actor.type === "character" || c.actor.type === "synthetic") && !c.actor.system.header.npc,
            )
            .sort((a, b) => {

	...
	static async yzeRoll(
		...
		if (r2Dice >= 1) {
			...
			if (
				...
				if (game.settings.get("alienrpg", "autopanic")) {
					if (game.settings.get("alienrpg", "evolved")) {
						if (tactorid !== "spacecraft") {
							...
							myActor.rollResolve(myActor, dataset)
						} else {
							myActor = game.actors.get(actorid)
				...
				chatMessage +=
					'<div class="alienchatblue warnblink"; style="font-weight: bold; font-size: larger">' +
					game.i18n.localize("ALIENRPG.supplyDecreases") +
					"</div>"
			}
		}
		// *******************************************************
				...
				case game.i18n.localize("ALIENRPG.Radiation"):
					{
						if (game.alienrpg.rollArr.r1Six + game.alienrpg.rollArr.r2Six + game.alienrpg.rollArr.sCount >= 1) {
							chatMessage +=
								...
								" " +
								localizedCountOfSuccesses(
									game.alienrpg.rollArr.r1Six + game.alienrpg.rollArr.r2Six + game.alienrpg.rollArr.sCount,
								) +
								" </div>"
						} else {
							chatMessage +=
		...
		function buildChat(mr, numDie, dType) {
			...
			if (dType !== "Stress") {
				for (let index = 0; index < mr.terms[0].results.length; index++) {
					const spanner = flattenObj(mr.terms[0].results[index])
				...
				chatMessage +=
					...
					r1Dice +
					" " +
					game.i18n.localize("ALIENRPG.Dice") +
					"</div>"
				chatMessage +=
					'<span <span class="alienchatlightgreen" style="font-size:larger"> ' +
					game.i18n.localize("ALIENRPG.Sixes") +
					"</span>"
				...
				if (
					actortype === "supply" ||
					(actortype !== "supply" && label === game.i18n.localize("ALIENRPG.RadiationReduced"))
				) {
					for (let index = 0; index < mr.terms[0].results.length; index++) {
						const spanner = flattenObj(mr.terms[0].results[index])
						numbers.push(spanner.result)
					...
					for (let index = 0; index < mr.terms[2].results.length; index++) {
						const spanner = flattenObj(mr.terms[2].results[index])
				...
				// Yellow Dice
				chatMessage +=
					'<div class="alienchatgoldenrod" style="font-size:larger">' +
					col2 +
					"  " +
					r2Dice +
					" " +
				...
				const _theRest = mrterms - (numOfY6s + numOfY1s)

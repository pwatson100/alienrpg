...
export class yze {
   ...
   *
   */
  static async yzeRoll(actortype, blind, reRoll, label, r1Dice, col1, r2Dice, col2, actorid, itemid, tactorid, moddata) {
      ...
      if (moddata && moddata.weapontype === "1") {
        ...
        for (let [skey, Attrib] of Object.entries(myActor.items.contents)) {
          if (Attrib.type === "talent" && Attrib.name === "Precise Shooter") {
            supplyModifier = -2;
            if (weapon.system.attributes.rounds.value + supplyModifier <= 0) {
              aveAmmo = 1 + "ds";
            } else {
        ...
        if (moddata.conserveammo === "true" && ammoOnes.length > 0) {
          chatMessage += `<div><span style="font-weight: bold; font-size: larger">` + game.i18n.localize("ALIENRPG.ammoConcerved") + "</span></div>";
        } else {
          chatMessage +=
            '<span class="alienchatlightgreen" style="font-size:larger">' +
            game.i18n.localize("ALIENRPG.ammo") +
            " " +

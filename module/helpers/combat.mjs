export default class AlienRPGCombat extends foundry.documents.Combat {
  /**
  ...
  async rollInitiative(ids, { formula = null, updateTurn = true, messageOptions = {} } = {}) {
    ...
    for (const [i, id] of ids.entries()) {
      ...
      if (drawn <= 10 && free > 0) {
        ...
        if (!game.settings.get("alienrpg", "alienrpgHideInitChat")) {
          ...
          if (!combatant.token.hidden || !combatant.hidden) {
            try {
              ...
              try {
                if (game.settings.get("alien-evolved-corerules", "imported")) {
                  cardPath = `<div style="text-align: center;"><img width="125" height="175" src="modules/alien-evolved-corerules/images/cards/card-${broll.total}.png"></div>`;
                }
              } catch (error) {}
              try {
                if (game.settings.get("alien-evolved-starterset", "imported")) {
                  cardPath = `<div style="text-align: center;"><img width="125" height="175" src="modules/alien-evolved-starterset/images/cards/card-${broll.total}.png"></div>`;
                }
              } catch (error) {}
            }
          }
        ...
        const cf = `1d9+( ${pNum})`;
        const broll = await this.getInit(combatant, cf, updates);
        updates.push({ _id: id, initiative: broll.total });
  ...
  _getExtraSpeedCombatants(combatant, data) {
    let creationData = {};
    const clones = [];
    const token = canvas.scene.tokens.get(combatant.tokenId);
    let creatureSpeed = 0;
    try {
      creatureSpeed = token.actor.system.attributes.speed.value;
    } catch (error) {
      const bob = game.actors.get(combatant.actorId);

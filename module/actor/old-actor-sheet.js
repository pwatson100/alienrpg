...
import { alienrpgrTableGet } from "./old-rollTableData.js"
/**
...
export class alienrpgActorSheet extends foundry.appv1.sheets.ActorSheet {
	...
	async getData(options) {
		...
		switch (this.actor.type) {
			case "character": {
				...
				const enrichedFields = ["system.notes", "system.adhocitems"]
			...
			case "creature": {
				...
				const enrichedFields2 = ["actor.system.notes"]
			...
			case "synthetic": {
				...
				await this.actor._checkOverwatch(data)
				await this._prepareItems(data)
				const enrichedFields3 = ["actor.system.notes", "actor.system.adhocitems"]
	...
	async _characterData(actor) {
		...
		for (const [skey, Attrib] of Object.entries(this.actor.items.contents)) {
			if (Attrib.system.attributes?.armorrating?.value && Attrib.system.header.active === true) {
				...
				Attrib.totalAc = Number.parseInt(Attrib.system.attributes.armorrating.value, 10)
			...
			if (Attrib.system.attributes?.water?.value && Attrib.system.header.active === true) {
				...
				Attrib.totalWat = Number.parseInt(Attrib.system.attributes.water.value, 10)
			...
			if (Attrib.system.attributes?.food?.value && Attrib.system.header.active === true) {
				...
				Attrib.totalFood = Number.parseInt(Attrib.system.attributes.food.value, 10)
			...
			if (Attrib.system.attributes?.airsupply?.value && Attrib.system.header.active === true) {
				...
				Attrib.totalAir = Number.parseInt(Attrib.system.attributes.airsupply.value, 10)
			...
			if (Attrib.system.attributes?.power?.value && Attrib.system.header.active === true) {
				...
				Attrib.totalPower = Number.parseInt(Attrib.system.attributes.power.value, 10)
			...
			if (Attrib.type === "item" || Attrib.type === "critical-injury" || Attrib.type === "armor") {
				if (Attrib.system.header.active === true) {
					...
					for (const [bkey, aAttrib] of Object.entries(base)) {
						switch (bkey) {
							case "str":
								attrMod.str = attrMod.str += Number.parseInt(aAttrib.value)
								break
							case "agl":
								attrMod.agl = attrMod.agl += Number.parseInt(aAttrib.value)
								break
							case "emp":
								attrMod.emp = attrMod.emp += Number.parseInt(aAttrib.value)
								break
							case "wit":
								attrMod.wit = attrMod.wit += Number.parseInt(aAttrib.value)
								break
							case "health":
								attrMod.health = attrMod.health += Number.parseInt(aAttrib.value)
								break
							case "stress":
								attrMod.stress = attrMod.stress += Number.parseInt(aAttrib.value)
								break
					...
					for (const [skkey, sAttrib] of Object.entries(skillBase)) {
						switch (skkey) {
							case "heavyMach":
								sklMod.heavyMach = sklMod.heavyMach += Number.parseInt(sAttrib.value)
								break
							case "closeCbt":
								sklMod.closeCbt = sklMod.closeCbt += Number.parseInt(sAttrib.value)
								break
							case "stamina":
								sklMod.stamina = sklMod.stamina += Number.parseInt(sAttrib.value)
								break
							case "rangedCbt":
								sklMod.rangedCbt = sklMod.rangedCbt += Number.parseInt(sAttrib.value)
								break
							case "mobility":
								sklMod.mobility = sklMod.mobility += Number.parseInt(sAttrib.value)
								break
							case "piloting":
								sklMod.piloting = sklMod.piloting += Number.parseInt(sAttrib.value)
								break
							case "command":
								sklMod.command = sklMod.command += Number.parseInt(sAttrib.value)
								break
							case "manipulation":
								sklMod.manipulation = sklMod.manipulation += Number.parseInt(sAttrib.value)
								break
							case "medicalAid":
								sklMod.medicalAid = sklMod.medicalAid += Number.parseInt(sAttrib.value)
								break
							case "observation":
								sklMod.observation = sklMod.observation += Number.parseInt(sAttrib.value)
								break
							case "survival":
								sklMod.survival = sklMod.survival += Number.parseInt(sAttrib.value)
								break
							case "comtech":
								sklMod.comtech = sklMod.comtech += Number.parseInt(sAttrib.value)
								break
			...
			if (
				...
				aData.attributes.wit.value > aData.attributes.emp.value
			) {
			...
			if (
				...
				aData.attributes.wit.value > aData.attributes.str.value
			) {
				aData.skills.stamina.ability = "wit"
			}
		}

		for (const [a, abl] of Object.entries(aData.attributes)) {
			const target = `system.attributes.${a}.mod`
			let field = `aData.attributes[${a}].mod`
			const upData = Number.parseInt(abl.value || 0) + Number.parseInt(attrMod[a] || 0)
			await this.actor.update({ [target]: (field = upData) })
			abl.mod = Number.parseInt(abl.value || 0) + Number.parseInt(attrMod[a] || 0)
			abl.label = CONFIG.ALIENRPG.attributes[a]
		}

		for (const [s, skl] of Object.entries(aData.skills)) {
			const conSkl = skl.ability
			const target = `system.skills.${s}.mod`
			let field = `aData.skills[${s}].mod`
			const upData =
				Number.parseInt(skl.value || 0) +
				Number.parseInt(aData.attributes[conSkl].mod || 0) +
				Number.parseInt(sklMod[s] || 0)
			await this.actor.update({ [target]: (field = upData) })
			skl.mod =
				Number.parseInt(skl.value || 0) +
				Number.parseInt(sklMod[s] || 0) +
		...
		await this.actor.update({
			...
			"system.consumables.water.value": (aData.consumables.water.value = Number.parseInt(totalWat || 0)),
			"system.consumables.food.value": (aData.consumables.food.value = Number.parseInt(totalFood || 0)),
			"system.consumables.air.value": (aData.consumables.air.value = Number.parseInt(totalAir || 0)),
			"system.consumables.power.value": (aData.consumables.power.value = Number.parseInt(totalPower || 0)),
			"system.general.armor.value": (aData.general.armor.value = Number.parseInt(totalAc || 0)),
			"system.general.radiation.calculatedMax": (aData.general.radiation.calculatedMax =
				Number.parseInt(aData.general.radiation.max - (aData.general.radiation.permanent ?? 0)) || 0),
			...
			"system.header.health.mod": (aData.header.health.mod = Number.parseInt(attrMod.health || 0)),
			"system.header.health.calculatedMax": (aData.header.health.calculatedMax =
				aData.attributes.str.value + aData.header.health.mod),
		})

		if (actor.actor.type === "character") {
			await this.actor.update({
				"system.header.stress.mod": (aData.header.stress.mod = Number.parseInt(attrMod.stress || 0)),
	...
	async _prepareTerritoryItems(data) {
		...
		// let totalWeight = 0;
		for (const i of data.system.items) {
			const item = i.system
			// Append to gear.
	...
	async _prepareCreatureItems(sheetData) {
		...
		// Iterate through items, allocating to containers
		for (const i of sheetData.system.items) {
			critInj.push(i)
		}
	...
	async _inlineedit(event) {
		...
		const itemId = dataset.parentElement.dataset.itemId
		const item = this.actor.items.get(itemId)
		const temp = dataset.dataset.mod
	...
	_stuntBtn(event) {
		event.preventDefault()
		const li = $(event.currentTarget).parents(".grid-container")
		const li2 = li.children("#panel")
		let item = ""
		let str = ""
		let chatData = ""
		let temp2 = ""
		...
		const div = $(`<div class="panel Col3">${chatData}</div>`)
		// Toggle summary
		if (li2.hasClass("expanded")) {
			const summary = li2.children(".panel")
	...
	async _onClickXPStatLevel(event) {
		event.preventDefault()
		const xp = this.actor.system.general.xp
	...
	async _onClickSPStatLevel(event) {
		event.preventDefault()
		const sp = this.actor.system.general.sp
		if (event.type == "contextmenu") {
			// left click
			if (sp.value > 0) {
				if (sp.value === 0) {

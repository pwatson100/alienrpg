...
export class alienrpgActor extends Actor {
  ...
  async abilityRoll(actor, dataset, rollMod) {
    ...
    if (dataset.roll) {
      ...
      if (attrib && actor.type !== "synthetic" && !rollMod) {
        ...
        content = await foundry.applications.handlebars.renderTemplate("systems/alienrpg/templates/dialog/roll-attr-dialog.hbs", actor, dataset);
        ...
        content = await foundry.applications.handlebars.renderTemplate("systems/alienrpg/templates/dialog/roll-space-comtech.hbs", { config, actor, dataset });
        response = await foundry.applications.api.DialogV2.wait({
          window: { title: title },
          content,
          rejectClose: false,
  ...
  async rollAbilityMod(actor, dataset) {
    ...
    if (dataset.roll) {
      ...
      if (
        ...
        content = await foundry.applications.handlebars.renderTemplate("systems/alienrpg/templates/dialog/roll-all-dialog.hbs", actor, dataset.dataset);
      } else if (actor.type === "synthetic") {
        content = await foundry.applications.handlebars.renderTemplate("systems/alienrpg/templates/dialog/roll-base-dialog.hbs", actor, dataset.dataset);
      } else {
        content = await foundry.applications.handlebars.renderTemplate("systems/alienrpg/templates/dialog/roll-base-dialog.hbs", actor, dataset.dataset);
      }
    }
  ...
  async rollPanicMod(actor, dataset) {
    const config = CONFIG.ALIENRPG;
    const content = await foundry.applications.handlebars.renderTemplate("systems/alienrpg/templates/dialog/roll-stress-dialog.hbs", actor, dataset.dataset);
    let response = "";
    const title = game.i18n.localize("ALIENRPG.DialTitle1") + " " + dataset.label + " " + game.i18n.localize("ALIENRPG.DialTitle2");
    let stressMod = 0;
    response = await foundry.applications.api.DialogV2.wait({
  ...
  async rollResolve(actor, dataset) {
    ...
    const html = await foundry.applications.handlebars.renderTemplate("systems/alienrpg/templates/chat/stress-response-roll.hbs", htmlData);
    const chatData = {
      user: game.user.id,
      speaker: {
        actor: actor.id,
  ...
  async rollResolveMod(actor, dataset) {
    const config = CONFIG.ALIENRPG;
    const content = await foundry.applications.handlebars.renderTemplate("systems/alienrpg/templates/dialog/roll-resolve-dialog.hbs", actor, dataset.dataset);
    let response = "";
    const title = game.i18n.localize("ALIENRPG.DialTitle1") + " " + dataset.label + " " + game.i18n.localize("ALIENRPG.DialTitle2");
    let resolveMod = 0;
    response = await foundry.applications.api.DialogV2.wait({
  ...
  async rollStress(actor, dataset) {
    ...
    const html = await foundry.applications.handlebars.renderTemplate("systems/alienrpg/templates/chat/panic-response-roll.hbs", htmlData);
    const chatData = {
      user: game.user.id,
      speaker: {
        actor: actor.id,
  ...
  async rollCrit(actor, type, dataset, manCrit) {
    ...
    switch (type) {
      case "character":
        {
          if (game.settings.get("alienrpg", "evolved")) {
            atable = game.tables.getName(game.i18n.localize("ALIENRPG.EVCriticalInjuries")) || game.tables.getName("EV - Critical Injuries");
          } else {
            atable =
    ...
    const html = await foundry.applications.handlebars.renderTemplate(`systems/alienrpg/templates/chat/crit-roll-${actor.type}.hbs`, htmlData);
    const chatData = {
      user: game.user.id,
      speaker: {
        actor: actor.id,
  ...
  async creatureAutoAttackRoll(actor, dataset, manCrit) {
    let chatMessage = "";
    let customResults = "";
    let roll = "";
  ...
  async creatureManAttackRoll(actor, dataset) {
    let content = "";
    let response = "";
    content = await foundry.applications.handlebars.renderTemplate("systems/alienrpg/templates/dialog/roll-manual-creature-attack-dialog.hbs", actor, dataset);
    response = await foundry.applications.api.DialogV2.wait({
    ...
    await this.creatureAutoAttackRoll(actor, dataset, response.manCrit);
  }
  async creatureAcidRoll(actor, dataset) {
    ...
    if (dataset.dataset.roll !== 0) {
      ...
      content = await foundry.applications.handlebars.renderTemplate("systems/alienrpg/templates/dialog/roll-base-xeno-dialog.hbs", actor, dataset.dataset);
      response = await foundry.applications.api.DialogV2.wait({
        window: { title: title },
        content,
        rejectClose: false,

...
export class alienrpgItem extends Item {
  ...
  async roll(right, dataset) {
    ...
    const title = game.i18n.localize("ALIENRPG.DialTitle1") + " " + label + " " + game.i18n.localize("ALIENRPG.DialTitle2");
    ...
    if (right) {
      ...
      if (this.actor.type === "character" || actorData.header.synthstress) {
        content = await foundry.applications.handlebars.renderTemplate("systems/alienrpg/templates/dialog/roll-all-dialog.hbs", { config, actorData, dataset });
        ...
        content = await foundry.applications.handlebars.renderTemplate("systems/alienrpg/templates/dialog/roll-base-dialog.hbs", config, actorData, dataset);
        response = await foundry.applications.api.DialogV2.wait({
          window: { title: title },
          content,
          rejectClose: false,
        ...
        switch (itemData.header.type.value) {
          case "1":
            if (game.settings.get("alienrpg", "evolved")) {
              ...
              content = await foundry.applications.handlebars.renderTemplate("systems/alienrpg/templates/dialog/roll-ranged-weapon-dialog.hbs", {
                config,
                actorData,
                dataset,
              });
          ...
          case "vehicles":
            {
              if (itemData.header.type.value === "1" || itemData.header.type.value === "2") {
                ...
                content = await foundry.applications.handlebars.renderTemplate("systems/alienrpg/templates/dialog/roll-vehicle-weapon.hbs", {
                  config,
                  actorData,
                  dataset,
                  options,
          ...
          case "spacecraft":
            {
              if (itemData.header.type.value === "1") {
                ...
                if (response) {
                  ...
                  const r1Data = Number(itemData.attributes.bonus.value + modifier + rangeMod + game.actors.get(tactorid).system.skills.rangedCbt.mod);
                ...
                content = await foundry.applications.handlebars.renderTemplate("systems/alienrpg/templates/dialog/roll-spacecraft-defense.hbs", {
                  config,
                  actorData,
                  dataset,
                  options,
  ...
  async rollComputerMod(item, dataset) {
    ...
    const content = await foundry.applications.handlebars.renderTemplate("systems/alienrpg/templates/dialog/roll-base-dialog.hbs", { item, dataset });
